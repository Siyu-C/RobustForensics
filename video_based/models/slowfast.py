...
class SlowFast(nn.Module):
    def __init__(self, block=Bottleneck, layers=[3, 4, 6, 3], num_classes=400, 
        ...
        self.fast_res1 = self._make_layer_fast(
            block, int(64 * beta), layers[0], head_conv=3)
        self.fast_res2 = self._make_layer_fast(
            block, int(128 * beta), layers[1], stride=2, head_conv=3)
        self.fast_res3 = self._make_layer_fast(
            block, int(256 * beta), layers[2], stride=2, head_conv=3)
        self.fast_res4 = self._make_layer_fast(
            block, int(512 * beta), layers[3], stride=2, head_conv=3)
        ...
        self.slow_res3 = self._make_layer_slow(
            block, 256, layers[2], stride=2, head_conv=3) # Here we add non-degenerate t-conv
        self.slow_res4 = self._make_layer_slow(
            block, 512, layers[3], stride=2, head_conv=3) # Here we add non-degenerate t-conv
    ...
    def _make_layer_fast(self, block, planes, blocks, stride=1, head_conv=1):
        ...
        layers = []
        layers.append(block(self.fast_inplanes, planes, stride, 
                            downsample, head_conv=head_conv))
    ...
    def _make_layer_slow(self, block, planes, blocks, stride=1, head_conv=1):
        ...
        layers = []
        layers.append(block(self.slow_inplanes + int(self.slow_inplanes * self.beta) * 2, planes, stride, 
                            downsample, head_conv=head_conv))
