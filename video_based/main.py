...
def main():
    ...
    spatial_transform = [
        ScaleJitteringRandomCrop(opt.min_scale, opt.max_scale, opt.input_size),
    ...
    if opt.get('jpegcompression', False):
        spatial_transform.append(ImageCompression(p=.2, quality_lower=50))
    ...
    train_sampler = DistributedGivenIterationSampler(train_dataset, config.lr_scheduler.max_iter, 
    ...
    with torch.no_grad():
        for i, (input, target, index) in enumerate(val_loader):
            ...
            if save_softmax:
                ...
                logits = output.data.cpu()
                for idx in range(len(index)):
                    cur_index = index[idx].item()
                    meta, _ = val_loader.dataset.lookup_meta(cur_index)
                    name, label = meta[0], meta[-1]
                    output_dict[cur_index] = {'filename': name,
                                              'prob': 1. - prob[idx][0].item(),
                                              'label': 0 if label == 0 else 1,
                                              'logits': logits[idx].numpy().tolist()}
                    
            prec1, prec2 = accuracy(prob.data, target, topk=(1, 2))
        ...
        if rank == 0:
            ...
            for idx in score_list:
                ...
                if vid not in video_score_list:
                    ...
                    if config.get('video_reduce_type', 'average') == 'max':
                        video_score_list[vid]['prob'] = max(video_score_list[vid]['prob'], prob)
                    else:
